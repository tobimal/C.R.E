class Admins::SeriesNovelsController < ApplicationController
  def index
  ...
  def create
    ...
    if @series_novel.save
        redirect_to admins_series_novels_path
      else
        @series_novels = SeriesNovel.all
        render :index
    end
  ...
  def update
    ...
    if @series_novel.update(series_novel_params)
      redirect_to admins_series_novel_path(@series_novel), notice: "You have updated series_novel successfully."
    else
      render "edit"
    end
  end
  def destroy
    ...
    redirect_to admin_series_novels_path
  end
